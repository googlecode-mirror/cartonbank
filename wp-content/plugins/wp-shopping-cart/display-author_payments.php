      ...
      foreach($product_list as $sales)
        {
            ...
            //paypal
            //actual_money
            if ($sales['gateway']=='paypal_multiple')
            {
                //$sales['discount'] = $sales['discount']+5; // average default discount for Robokassa
                $paypal_text = ' paypal: 5% ';

                if ($sales['actual_money']=='')
                {
                    $sales['actual_money'] = ($sales['price']*0.95); // average money received from PayPal
                }
                else
                {
                    $sales['actual_money'] = $sales['actual_money']; //assign corrected by accountant discount to PayPal purchase
                }
            ...
            // pokazh($sales);
            // Dead artist correction. Melnik
            if ($artist['id']==7 && (int)$y > 2012 && (int)$_month > 9){

                echo "<div class='t' style='font-size:0.8em;'><span style='color:silver;'>".$sales['datetime']."</span> Заказ:".$sales['purchaseid']." №:".$sales['picture_id']." <b>".$sales['smi']."</b> <i>".$sales['firstname']." ".$sales['lastname']."</i> «".stripslashes($sales['title'])."» цена:".round($sales['price'],0).$robokassa_text.$paypal_text." скидка:".round($sales['discount'],0)."% итого:<b>".$discount_price."</b><span style='color:#9900CC;'> Автору: ".round(0.6*($discount_price),0)." руб.</span></div>";
                $total=$total + round(0.6*($discount_price),0);
            }
            else {
        ...
        // Dead artist correction. Melnik
        if ($artist['id']==7 && (int)$y > 2012 && (int)$_month > 9){
            echo "<div style='color:#9900CC;margin-bottom:10px;'>Всего авторских: <b>".$total."</b>. Специальная доля автора 60%</div>";
        }
        else {
