...
function nzshpcrt_install()
  ...
  if(!in_array($table_name, $tables))
	...
	$submitted_data_table = "CREATE TABLE `".$wpdb->prefix."submited_form_data` (
	 ...
	 //$coldata  = $db->GetAll("SHOW FULL COLUMNS FROM `$tablename` LIKE '$col'");
	 
	 //$addcolsql = "ALTER TABLE `$tablename` ADD `$col` TEXT NOT NULL $after\n\r";
	  ...
	  */     
	  ...
	  $currency_data  = $wpdb->get_results("SELECT COUNT(*) AS `count` FROM `".$wpdb->prefix."currency_list`",ARRAY_A);
	  if($currency_data[0]['count'] == 0)
		{
...
<!-- 
<script src="http://ant.cartoonist.name/cb/wp-content/plugins/wp-shopping-cart/js/prototype.js" language='JavaScript' type="text/javascript"></script>
<script src="http://bat.cartoonist.name/cb/wp-content/plugins/wp-shopping-cart/js/scriptaculous.js?load=effects" language='JavaScript' type="text/javascript"></script>
<script src="http://cat.cartoonist.name/cb/wp-content/plugins/wp-shopping-cart/js/lightbox.js" language='JavaScript' type="text/javascript"></script>
 -->
...
function nzshpcrt_submit_ajax()
  ...
  if(($_GET['ajax'] == "true") && ($_GET['user'] == "true") && is_numeric($_POST['prodid']))
    ...
    //mail("aleshin@dataart.com","test",print_r($_REQUEST,true));
...
function nzshpcrt_getproductform($prodid)
  ...
  */ 
  ...
  $output .= "            </td>\n\r";
  $output .= "            <td>\n\r";
  $output .= "<textarea name='description' cols='30' rows='2' >".stripslashes($product['description'])."</textarea>";
  $output .= "            </td>\n\r";
  $output .= "          </tr>\n\r";
  ...
  $output .= "            </td>\n\r";
  $output .= "            <td>\n\r";
  $output .= "<textarea name='additional_description' cols='30' rows='3' >".stripslashes($product['additional_description'])."</textarea>";
  $output .= "            </td>\n\r";
  $output .= "          </tr>\n\r";
...
//  $output .= "    </tr>\n\r";
  ...
  $output .= "          <tr>\n\r";
  $output .= "            <td colspan='2'>\n\r";
  $output .= "<strong class='form_group'>".TXT_WPSC_PRODUCT_VARS."</strong>";
  $output .= "            </td>\n\r";
  $output .= "          </tr>\n\r";
...
//  $output .= "          </tr>\n\r";
  ...
  if($product['file'] > 0)
    ...
    $output .= "          <tr>\n\r";
    $output .= "            <td colspan='2'>\n\r";
    $output .= "<strong class='form_group'>".TXT_WPSC_PRODUCTDOWNLOAD."</strong>";
    $output .= "            </td>\n\r";
    $output .= "          </tr>\n\r";
    ...
    $output .= "            </td>\n\r";
    $output .= "            <td>\n\r";
    $output .= "<input type='file' name='file' value='' /> <span class='small'><br />".TXT_WPSC_FILETOBEPRODUCT."</span><br />";
    $output .= "            </td>\n\r";
    $output .= "          </tr>\n\r";
    }
  $output .= "          <tr>\n\r";
...
//    }
   ...
   foreach($cart as $cart_item)
      ...
      if($product_data['special']==1)
        ...
        if($product_data[0]['notax'] == 1)
          ...
          $price = ($product_data['price'] - $price_modifier);
            ...
            {
            $price = (($product_data['price'] - $price_modifier) * get_option('gst_rate'));
            }
    ...
    if($country == get_option('base_country'))
     ...
     $cartsql = "INSERT INTO `".$wpdb->prefix."cart_contents` ( `id` , `prodid` , `purchaseid`, `price`, `pnp`,`quantity` ) VALUES ('', '".$row."', '".$getid[0]['id']."','".$price."','".$shipping."','".$quantity."')";
  ...
  // ales if(($_POST['payment_method'] == 2) && (get_option('payment_method') == 2))
if(($_POST['payment_method'] == 1))
    {
	echo ("PrePaid: <pre>".print_r($_POST,true)."</pre>");
	}
  if(($_POST['payment_method'] == 2))
    {
	  echo ("PayPal: <pre>".print_r($_POST,true)."</pre>");
//	  echo "curgateway=".$curgateway;
//	  echo "gateway=".$gateway;
//	  exit("<pre>".print_r($_POST,true)."</pre>");
    foreach($nzshpcrt_gateways as $gateway)
      {
      if($gateway['internalname'] == 'testmode' )
		  exit($gateway['internalname']);
        {
...
//	  echo "curgateway=".$curgateway."<br>";
//	  echo "gateway=".$gateway."<br>";
//	  exit ("<pre>".print_r($gateway,true)."</pre>");
//	  exit($gateway['internalname']);
...
function al_your_download_limit(){
	global $userdata;
	if (isset($userdata))
	{
		$limit_data = al_get_limit($userdata->ID); 
...
function nzshpcrt_shopping_basket_internals($cart,$quantity_limit = false, $title='')
  ...
  switch(get_option('cart_location'))
    ...
    $output .="<span id='alt_loadingindicator'><img id='alt_loadingimage' src='". get_option('siteurl')."/wp-content/plugins/wp-shopping-cart/images/indicator.gif' alt='Loading' title='Loading' /> ".TXT_WPSC_UDPATING."...</span></strong>";
    $spacing = "";
    break;
    
    case 3:
    $output .= "<h2>".TXT_WPSC_SHOPPINGCART."</h2>";
    $output .= al_your_download_limit();
    $spacing = "<br/><br />";
    break;
  ...
  if($cart != null)
	...
	$output .= "<tr><th>".TXT_WPSC_PRODUCT."</th><th>".TXT_WPSC_PRICE."</th></tr>";
    foreach($cart as $cart_item)
      {
      ...
      $product = $wpdb->get_results($sql,ARRAY_A);
      if($product[0]['special']==1)
        {
        $price_modifier = $product[0]['special_price'];
        }
        else
          {
          $price_modifier = 0;
          }
      $price = nzshpcrt_currency_display(($quantity * ($product[0]['price']-$price_modifier)), $product[0]['notax'],false,false,true);
	  ...
	  $output .= "<tr><td>".$product[0]['name']."</td><td>".nzshpcrt_currency_display($price, 1)."</td></tr>";
    ...
    if($_SESSION['selected_country'] != null)
      ...
      $output .= "<strong>".TXT_WPSC_TOTAL.":</strong> &nbsp;&nbsp;".nzshpcrt_currency_display(($total+$total_shipping), 1)."<br /><br />";
      }
      else
        {
		 //ales
        //$output .= "<strong>".TXT_WPSC_TOTAL.":</strong> &nbsp;&nbsp;".nzshpcrt_currency_display($total, 1)." + ".TXT_WPSC_POSTAGE."<br /><br />";
		$output .= "<strong>".TXT_WPSC_TOTAL.":</strong> &nbsp;&nbsp;".nzshpcrt_currency_display($total, 1)."<br /><br />";
        }
    if(get_option('permalink_structure') != '')
      {
      $seperator ="?";
