			...
			// All words search match
                ...
                if((isset($_POST['cs']) && $_POST['cs']!= '') or (isset($_GET['cs']) && $_GET['cs']!= '') or (isset($_POST['cs_exact']) && $_POST['cs_exact']!= '') or (isset($_GET['cs_exact']) && $_GET['cs_exact']!= '') or (isset($_POST['cs_any']) && $_POST['cs_any']!= '') or (isset($_GET['cs_any']) && $_GET['cs_any']!= ''))
                {
					...
					if (isset($_brand) && isset($_brand)!='')
					{
						...
						$search_sql = "SELECT COUNT(*) as count  FROM `wp_product_list`, `wp_product_files`, `wp_product_brands`, `wp_product_categories` WHERE `wp_product_list`.`active`='1' " . $cat_group_sql . $exclude_category_sql . $brand_group_sql . $colorfilter . $approved_or_not . " AND `wp_product_list`.`visible`='1' ".$search_keywords_filter." AND `wp_product_list`.`file` = `wp_product_files`.`id` AND `wp_product_brands`.`id` = `wp_product_list`.`brand` AND `wp_product_list`.`category` = `wp_product_categories`.`id`";
                    ...
                    else
					{
						...
						$search_sql = "SELECT COUNT(*) as count  FROM `wp_product_list`, `wp_product_files`, `wp_product_brands`, `wp_product_categories` WHERE `wp_product_list`.`active`='1' " . $cat_group_sql . $exclude_category_sql . $colorfilter . $approved_or_not . " AND `wp_product_list`.`visible`='1' ".$search_keywords_filter." AND `wp_product_list`.`file` = `wp_product_files`.`id` AND `wp_product_brands`.`id` = `wp_product_list`.`brand` AND `wp_product_list`.`category` = `wp_product_categories`.`id`";
					}
    ...
    //$product = $GLOBALS['wpdb']->get_results($sql,ARRAY_A);
     ...
     {   
			...
			// slide preview preparations:
				...
				$_bigpicstrip = "<div style=\"float:left;\"><b><a href='".get_option('siteurl')."/?page_id=278' target=_blank title='объяснение'>Название:</a> </b>" .$_name."</div> "."<div >№&nbsp;<a title='уникальный адрес страницы с этим изображением' id='cuid' href='".get_option('siteurl')."/cartoon/".$_number."/'>".$_number."</a>&nbsp;<b><a href=\"".$siteurl."/?page_id=29&brand=".$_brandid."\">".$_author."</a></b></div>";
