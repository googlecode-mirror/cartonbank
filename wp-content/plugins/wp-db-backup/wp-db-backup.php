		...
		if (is_writable($this->backup_dir)) {
			...
			if(!$this->fp) {
				...
				if($table == '') {		
					...
					$this->stow("# " . __('Cartoonbank MySQL database backup','wp-db-backup') . "\n");
					$this->stow("#\n");
...
//		$dir_perms = $stat['mode'] & 0000777; // Get the permission bits.
		...
		// not writable due to write permissions
			...
			// the directory is not writable probably due to safe mode
				...
				if( ini_get('safe_mode') ){
					?><p><?php _e('This problem seems to be caused by your server&rsquo;s <code>safe_mode</code> file ownership restrictions, which limit what files web applications like Cartoonbank can create.', 'wp-db-backup'); ?></p><?php 
				}
		...
		<h4><?php _e('These core Cartoonbank tables will always be backed up:','wp-db-backup') ?></h4><ul><?php
		$excs = (array) get_option('wp_db_backup_excs');
	...
	function wp_secure($kind = 'warn', $loc = 'main') {
		...
		if ( function_exists('wp_verify_nonce') ) return true;
			...
			$this->error(array('kind' => $kind, 'loc' => $loc, 'msg' => sprintf(__('Your Cartoonbank version, %1s, lacks important security features without which it is unsafe to use the WP-DB-Backup plugin.  Hence, this plugin is automatically disabled.  Please consider <a href="%2s">upgrading Cartoonbank</a> to a more recent version.','wp-db-backup'),$wp_version,'http://wordpress.org/download/')));
			return false;
		}
	}
