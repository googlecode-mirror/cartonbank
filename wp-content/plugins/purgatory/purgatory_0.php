...
<?php
	...
	//pokazh(ABSPATH.'/wp-content/RGrid/RGrid.php'); //Z:\home\localhost\www//wp-content/RGrid/RGrid.php

    require_once(WP_CONTENT_DIR.'/RGrid/RGrid.php');
	...
	// all not approved pics
    $sql = "SELECT wp_product_list.id as ID, wp_product_list.image AS image, wp_product_list.name AS title, wp_product_brands.name AS author, COUNT(V.vote_id) AS votes, SUM(V.vote_value) AS points
FROM wp_product_list, wp_product_brands, al_editors_votes as V
WHERE wp_product_list.brand = wp_product_brands.id
	AND V.image_id = wp_product_list.id
	AND wp_product_list.active =1
	AND wp_product_list.visible =1
	AND wp_product_list.approved = False";
//pokazh ($sql);
//$sql = "SELECT * FROM `wp_fsr_post` as p, `wp_fsr_user` as u WHERE p.id = u.post order by vote_date desc";
    ...
    */
    ...
    $grid->NoSpecialChars('ID', 'image','title','5');
    ...
    */
    ...
    function RowCallback(&$row)
    {
		...
		$row['5'] = '<input type="button" value="Yes" > <input type="button" value="No" >';
