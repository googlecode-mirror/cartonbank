...
<body class="login">
<?php   if ( !is_multisite() ) { ?>
<div id="login"><h1><a href="<?php echo apply_filters('login_headerurl', get_option('siteurl')); ?>" title="<?php echo apply_filters('login_headertitle', __('Powered by Cartoonbank')); ?>"><?php bloginfo('name'); ?></a></h1>
<?php   } else { ?>
...
<p>
<ul>
<li>Все сведения, предоставленные Пользователем при регистрации в учетной системе Банка Изображений, должны быть достоверными.
Пользователь отвечает за правильность, достоверность и полноту передаваемых сведений. При использовании недостоверных и/или неполных сведений, полученных от Пользователя, Банк Изображений не несет ответственности за негативные последствия, вызванные предоставлением недостоверных и/или неполных сведений.
<li>При регистрации в учетной системе Банка Изображений Пользователь получает индивидуальный административный логин и пароль доступа в свой Личный кабинет, самостоятельно обеспечивает их конфиденциальность, несет ответственность за все действия, произведенные с использованием логина и пароля.
<li>Пользователь несет полную ответственность за сохранность своих учетных данных (логина и пароля) и за убытки или иной ущерб, которые могут возникнуть по причине несанкционированного использования этой информации. По факту утери или несанкционированного доступа к учетным данным или возможности возникновения такой ситуации Пользователь обязан незамедлительно направить запрос Банку Изображений на смену учетных данных. В этом случае Банк Изображений при необходимости и по согласованию с Пользователем может осуществить блокировку доступа к Личному кабинету Пользователя.
<li>В случае если Пользователь предоставляет третьим лицам доступ к своему Личному кабинету, имеющимся у него ресурсам и сервисам, ответственность за деятельность таких третьих лиц несет исключительно Пользователь.
<li>При почтовых отправлениях от Банка Изображений к Пользователю используется почтовый адрес, указанный в Личном кабинете Пользователя на момент формирования почтового отправления.
</ul>
</p>
...
<?php		exit;
	...
	if ( isset($_POST['testcookie']) && empty($_COOKIE[TEST_COOKIE]) )
		$errors->add('test_cookie', __("<strong>ERROR</strong>: Cookies are blocked or not supported by your browser. You must <a href='http://www.google.com/cookies.html'>enable cookies</a> to use Cartoonbank."));
