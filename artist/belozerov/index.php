...
<html>
 <head>
  ...
  <title> Список работ Сергея Белозёрова </title>
	<style>
	.t{
 ...
 <body>
<h1>Список работ Сергея Белозёрова. Карикатуры. </h1>
...
{$p=5000;}
	...
	$sql = "SELECT `id`,`name`,`description`,`additional_description` FROM `wp_product_list` where `active`=1 and `brand`= 48 order by id LIMIT ".($p-1000)." , 1000";
///pokazh($sql);
