		 ...
		 else
		 {
			...
			// slide preview preparations:
				...
				else
				{
					...
					//$_edid .= "<iframe src='http://www.facebook.com/plugins/like.php?app_id=225580560788972&amp;href=".$_number."&amp;send=false&amp;layout=button_count&amp;width=150&amp;show_faces=true&amp;action=like&amp;colorscheme=light&amp;font&amp;height=21' scrolling='no' frameborder='0' style='border:none; overflow:hidden; width:150px; height:21px;' allowTransparency='true'></iframe>";

					$_edid .= "<div id='fb-root'></div><script src='http://connect.facebook.net/en_US/all.js#appId=130643610343080&amp;xfbml=1'></script><fb:like href='".$url."' send='false' layout='button_count' width='200' show_faces='true' font=''></fb:like>";
				}

				$_bigpicstrip = "<div style=\"float:left;\"><b><a href='".get_option('siteurl')."/?page_id=278' target=_blank title='объяснение'>Название:</a> </b><span id='cname'>" .$_name."</span></div> "."<div >№&nbsp;<a title='уникальный адрес страницы с этим изображением' id='cuid' href='".get_option('siteurl')."/?page_id=29&cartoonid=".$_number."'>".$_number."</a>&nbsp;<b><a href=\"".$siteurl."/?page_id=29&brand=".$_brandid."\"><span id='cauthor'>".$_author."</span></a></b></div>";
                ...
                $siteurl = get_option('siteurl');
                $_bigpic =  "<img src='".$siteurl."/wp-content/plugins/wp-shopping-cart/product_images/".$product[0]['image']."' border=0 id='cbig' alt='".$_bigpicimgalt."'>";
...
function set_title()
{
	var tit = jQuery('#cname').text();
	var aut = jQuery('#cauthor').text();
	document.title = tit+ '. ' + aut;
}

function setOgMeta()
{
	var cuid = document.getElementById('cuid').innerHTML;
	var src= jQuery('#cbig').attr('src');
	setOrCreateMetaTag('property', 'og:title', document.title);
	setOrCreateMetaTag('property', 'og:type', 'product');
	setOrCreateMetaTag('property', 'og:url', 'http://cartoonbank.ru/?page_id=29&amp;cartoonid='+cuid);
	setOrCreateMetaTag('property', 'og:image', src);
	setOrCreateMetaTag('property', 'og:site_name', 'Cartoonbank');
	setOrCreateMetaTag('property', 'fb:admins', '1021562104');
}
