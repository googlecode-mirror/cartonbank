...
{
	echo ("write to file");
	write_tags_to_file_small();
}
else
{
	read_tags_to_browser_small();
}
function read_tags_to_browser_small()
{
	$filename = '/home/www/cb3/wp-content/tags/tagcloud_small.htm';
...
function write_tags_to_file_small()
	{
	$filename = '/home/www/cb3/wp-content/tags/tagcloud_small.htm';

	$somecontent = create_tag_cloud_small();
...
function create_tag_cloud_small()
	{
	$sql = "SELECT DISTINCT `additional_description` AS name FROM `wp_product_list` WHERE `active` = 1 AND `visible` = 1 AND `additional_description` > '' ORDER BY id DESC LIMIT 5000";
	...
	//$result_array = array_map('trim',$result_array);
		...
		$result_string = r_implode_small(",",$result_array);
		$result_string = str_replace(".",",",$result_string); 
		$result_string = str_replace(",,",",",$result_string); 
		$result_string = str_replace(", ","|",$result_string);
		$result_string = str_replace(",","|",$result_string);
		$tags = explode ("|",$result_string);
		array_walk($tags, 'edit_value_small');
    ...
    class wordCloudSmall
    {
		...
		function wordCloudSmall($words = false)
			{
...
function r_implode_small( $glue, $pieces )
	{
	  foreach( $pieces as $r_pieces )
	  {
		if( is_array( $r_pieces ) )
		{
		  $retVal[] = r_implode_small( $glue, $r_pieces );
		}
		else
		{
		  $retVal[] = $r_pieces;
...
function edit_value_small(&$value) 
	{ 
		$value = trim($value); 
