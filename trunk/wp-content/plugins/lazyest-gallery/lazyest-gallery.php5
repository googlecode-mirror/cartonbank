...
require_once('lazyest-cache.php5');
...
add_action('wp_head', 'add_lazyest_gallery_style5');
add_filter('the_content', 'parse_page_for_gallery5');

// Insert the gallery_add_pages5() sink into the plugin hook list for 'admin_menu'
add_action('admin_menu', 'gallery_add_pages5');

// Filter for the post with images links to the gallery ie:[[Image:folder/img.png|align|110|descr]]
add_action('wp_head', 'add_post_thumbs_style5');
add_filter('the_content', 'parse_posts_for_images5');
...
add_filter('admin_footer', 'lazyest_write_quicktag5');

// ============== Gallery Main Functions ===============

function showGallery5() { 				// Builds main gallery page
...
function parse_page_for_gallery5($content){
	global $wpdb, $post;
...
function add_lazyest_gallery_style5(){ ?>
...
function lazyestInit5() {					// Define variables values
...
function some_lightbox_plugin5(){
	if (
...
function some_thickbox_plugin5() {
	if (
...
function gallery_add_pages5() {		// Calls the admin panel file
	global $user_level;

	// gallery_add_pages5() is the sink function for the 'admin_menu' hook
...
function lazyest_write_quicktag5() {
 ...
 * call instead clean_folder_caption5()
 */

function get_folder_caption5($dir) {
	global $gallery_root;
...
function clean_folder_caption5($dir, $verse = true){

	if (!($xml_caption = get_folder_caption5($dir))){
		return false;
	}
 ...
 * NOTE: This is an "util" function, so do not call directly,
 * call instead clean_image_caption5()
 * NOTE: This function returns an array
 */

function get_image_caption5($imagedir) {
	global $gallery_root;
...
function clean_image_caption5($img, $dir, $verse = true){

	if (!($images = get_image_caption5($dir))){
		return false;
	}
...
function get_minimum_folder_level5($dir){
...
function lg_user_can_access5($folder){
	global $user_level;
	...
	$folder_level = get_minimum_folder_level5($folder);
	if (($user_level >= $folder_level || $folder_level == 1)  && lg_user_can_access5($top)) {
		return true;
	} else {
		return false;
	}
...
function parse_posts_for_images5($content){
	...
	if(strpos(strtolower($content), "[[image") === false || strpos(strtolower($content), "<code>[[image") == true) {
		...
		foreach ($imagedata as $imgdata){
			...
			if (isset($imgdata)) {
				...
				$title = $alt;
				if (strlen($title) == 0){
					$caption = clean_image_caption5($img, $folder);
				...
				* ============= */
				...
				// Lightbox code
				if ($lb_enabled == "TRUE" && $lb_posts == "TRUE" && $lg_scache == "TRUE" && some_lightbox_plugin5() || $lb_force == "TRUE") {
					$url .= "<a href='$urlImg' rel='lightbox' title='$title' class='internal'>";
				}
				// Thickbox code
				elseif ($tb_enabled == "TRUE" && $tb_posts == "TRUE" && $lg_scache == "TRUE" && some_thickbox_plugin5() || $tb_force == "TRUE") {
...
function add_post_thumbs_style5(){
	?>
...
function lg_list_folders5($title, $root = ''){
	...
	if ($dir_handler = opendir($root)) {
		...
		while ($file = readdir($dir_handler)) {
			...
			if ($user_level < get_minimum_folder_level5($file))
			...
			if (!in_array($file, $forbidden)) {
				...
				if (is_dir($root.$file.'/')) {
					...
					lg_list_folders5($title, $root.$file.'/');
				}
			}
			if (!in_array($file, $forbidden) && is_dir($root.$file.'/')) {
				echo "\n</li></ul>\n\t";
...
function lg_random_image5($title){
	...
	if ($dir_content = opendir($gallery_root)) {
		...
		if( file_exists($gallery_root.$folder.'captions.xml')){
			$caption = clean_image_caption5($img_file, $folder);
		}
		...
		// Lightbox code
		if ($lb_enabled == "TRUE" && $lb_sidebar == "TRUE" && $lg_scache == "TRUE" && some_lightbox_plugin5() || $lb_force == "TRUE") {
			echo "<a href='$urlImg' rel='lightbox' title='$title'>";
		}
		// Thickbox code
		elseif ($tb_enabled == "TRUE" && $tb_sidebar == "TRUE" && $lg_scache == "TRUE" && some_thickbox_plugin5() || $tb_force == "TRUE") {
...
function lg_build_img_array5($root = ''){
	global $gallery_root, $user_level;
	...
	if ($dir_handler = opendir($root)) {
		...
		while ($file = readdir($dir_handler)) {
			...
			if (!in_array($file, $forbidden)) {
				// Do not remove the trailing slash (/)
				if (is_dir($root.$file.'/')) {
					$images = array_merge($images, lg_build_img_array5($root.$file.'/'));
				} else {
...
function lg_build_folders_array5($root = ''){
	global $gallery_root, $user_level;
	...
	if ($dir_handler = opendir($root)) {
		...
		while ($file = readdir($dir_handler)) {
			...
			if (!in_array($file, $forbidden)) {
				...
				if (is_dir($root.$file.'/')) {
					$images[] = $root.$file;
					$images = array_merge($images, lg_build_folders_array5($root.$file.'/'));
				} else {
					continue;
				}
			}
