...
require_once('lazyest-gallery.php5');
global $gallery_root;
define('AL_FLM_PAGE', 'admin.php?page=lazyest-gallery/mass-upload.php5');
	...
	if (isset($imgfiles)) {
		foreach ($imgfiles as $img) {
			...
			if (get_option('lg_enable_cache') == "TRUE"){
				...
				$righturl = str_replace(" ", "%20", get_settings('siteurl')."/wp-content/plugins/lazyest-gallery/".basename(__FILE__).".php5?file=". $act_current.$img."&amp;thumb=1");
				echo "<img src='".$righturl."' alt='".$img."' title='".$title."' />";
			}
		...
		global $wpdb, $mimetype;
		{
			...
			if(function_exists("getimagesize"))
			  ...
			  copy($img_path, $imagedir.$file_name);
					...
					$wm = "/homepages/35/d89900836/htdocs/cb/wp-content/plugins/wp-shopping-cart/images/watermark.gif";
